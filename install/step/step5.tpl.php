...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#password").formValidator({onshow:"6到20个字符<font color='FFFF00'>（默认为 12345678</font>",onfocus:"密码合法长度为6至20位"}).inputValidator({min:6,max:20,onerror:"密码合法长度为6至20位"});
	...
	<div class="content">
    	<table width="100%" cellspacing="1" cellpadding="0">
			  ...
			  <tr>
				...
				<td><input name="password" type="password" id="password" value="12345678" class="input-text" /></td>
			  </tr>
			  <tr>
				<th align="right">确认密码：</th>
				<td><input name="pwdconfirm" type="password" id="pwdconfirm" value="12345678" class="input-text" /></td>
			  </tr>
			  <tr>
...
errmsg[0] = '您已经安装过本系统，系统会自动删除老数据！是否继续？';
errmsg[2] = '无法连接数据库服务器，请检查配置！';
errmsg[3] = '成功连接数据库，但是指定的数据库不存在并且无法自动创建，请先通过其他方式建立数据库！';
errmsg[6] = '数据库版本低于Mysql 4.0，无法安装系统，请升级数据库版本！';
