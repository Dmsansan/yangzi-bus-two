...
class company {
	...
	function add(){
		...
		$sql="insert into company (company_name,remark)
		values ('$company_name','$remark')";
		$this->conn->query($sql);
		if($this->conn->affected_rows()>0){
			...
			$arr = array ('status'=>'ERROR','reason'=>'添加分公司失败！');
			echo json_encode($arr);
			die();
			//$this->log->do_log($str);
		}
	...
	*/
	function qry(){
		...
		$sql_cnt="select count(*) as cnt from company";
		$where="";
		/*if($brand_no!=""){
			if($where=="")
				$where=" where";
