...
　　 }
	...
	function deleteRow()
       { 
			...
			if (row) {
                $.ligerDialog.confirm("记录删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/v1/?menuid=101112&cmd=del", type: "POST",
                            data: { brand_id: row.brand_id, rnd: Math.random() },
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('module_sys/sys.brand_add.php?r_brand_id=' +row.brand_id, "修改记录", 770, 490, f_save);
            } else {
		...
		function itemclick(item) {
			switch (item.id) {
				case "add":
                  ...
                  //alert("增加");
				   f_openWindow("module_sys/sys.brand_add.php", "新增轮胎品牌", 770, 300, f_save);
                  break;
				case "clearbtn":
					deleteRow();
                  //var selected = grid.getSelected();
	  ...
	  function f_reload() {
		...
		function doserch() {
			...
			manager.set({url:'../ajaction/v1/?menuid=101112&cmd=qry&'+serchtxt});
			manager.loadData(true);
        }
            ...
            <table style='width: 960px' class="bodytable1">
                <tr>
                    <td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>品牌代码：</div>
						<div style='float: left;'>
						<input type='text' id='brand_no' name='brand_no' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
					<td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>品牌名称：</div>
						<div style='float: left;'>
						<input type='text' id='brand_name' name='brand_name' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
                    <td style='align:left'>
