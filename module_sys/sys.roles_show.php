...
　　 }
	...
	function deleteRow()
       { 
			...
			var row=manager.getSelectedRow();
			if (row) {
                $.ligerDialog.confirm("角色删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/v1/?menuid=101010&cmd=del", type: "POST",
                            data: { role_id: row.role_id, rnd: Math.random() },
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('module_sys/sys.roles_add.php?r_role_id=' +row.role_id, "修改角色", 770, 490, f_save);
            } else {
		...
		function itemclick(item) {
			switch (item.id) {
				case "add":
                  ...
                  //alert("增加");
				   f_openWindow("module_sys/sys.roles_add.php", "新增角色", 770, 300, f_save);
                  break;
				case "clearbtn":
					deleteRow();
                  //var selected = grid.getSelected();
	  ...
	  function f_reload() {
		...
		function doserch() {
			...
			manager.set({url:'../ajaction/v1/?menuid=101010&cmd=qry&'+serchtxt});
			manager.loadData(true);
        }
        ...
        $(function () {
            ...
            manager=$("#maingrid4").ligerGrid({
                checkbox: false,
                columns: [
                { display: '角色编号', name:'role_id', align: 'left', width: 60 },
                { display: '角色名称', name:'title', width:150,isSort:false },
				{ display: '模块列表', name:'modules_list', width: 300,align:'left',isSort:false },
            ...
            <table style='width: 960px' class="bodytable1">
                <tr>
                    <td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>角色名称：</div>
						<div style='float: left;'>
						<input type='text' id='title' name='title' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>					
                    <td style='align:left'>
