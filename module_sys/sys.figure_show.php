...
　　 }
	...
	function deleteRow()
       { 
			...
			if (row) {
                $.ligerDialog.confirm("记录删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/v1/?menuid=101117&cmd=del", type: "POST",
                            data: { figure_id: row.figure_id, rnd: Math.random() },
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('module_sys/sys.figure_add.php?r_figure_id=' +row.figure_id, "修改记录", 770, 490, f_save);
            } else {
		...
		function itemclick(item) {
			switch (item.id) {
				case "add":
                  ...
                  //alert("增加");
				   f_openWindow("module_sys/sys.figure_add.php", "新增轮胎花纹", 770, 300, f_save);
                  break;
				case "clearbtn":
					deleteRow();
                  //var selected = grid.getSelected();
	  ...
	  function f_reload() {
		...
		function doserch() {
			...
			manager.set({url:'../ajaction/v1/?menuid=101117&cmd=qry&'+serchtxt});
			manager.loadData(true);
        }
    ...
    remark          varchar(200)        comment '备注',
			...
			*/	  
            ...
            manager=$("#maingrid4").ligerGrid({
                checkbox: false,
                columns: [
                { display: '花纹编号', name:'figure_id', align: 'left', width: 60 },
                { display: '花纹代码', name:'figure_no', width:150,isSort:false },
				{ display: '花纹名称', name:'figure_name', width: 150},
				{ display: '备注',name:'remark',width: 300}
				], pageSize:10,
                url:'../ajaction/v1/?menuid=101117&cmd=qry&t=1',
            ...
            <table style='width: 960px' class="bodytable1">
                <tr>
                    <td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>花纹代码：</div>
						<div style='float: left;'>
						<input type='text' id='figure_no' name='figure_no' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
					<td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>花纹名称：</div>
						<div style='float: left;'>
						<input type='text' id='figure_name' name='figure_name' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
                    <td style='align:left'>
