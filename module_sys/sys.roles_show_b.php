...
　　 }
	...
	function deleteRow()
       { 
			...
			if (row) {
                $.ligerDialog.confirm("联系人删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/sysaction/sys.roles_grid_del.php", type: "POST",
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('CRM/Customer/Customer_add.aspx?role_id=' +row.role_id, "修改角色", 770, 490, f_save);
            } else {
	  ...
	  function f_reload() {
        ...
        $(function () {
            ...
            manager=$("#maingrid4").ligerGrid({
				...
				{ display: '模块列表', name:'modules_list', width: 300,align:'left' },
