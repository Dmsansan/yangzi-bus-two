...
　　 }
	...
	function deleteRow()
       { 
			...
			if (row) {
                $.ligerDialog.confirm("记录删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/v1/?menuid=101110&cmd=del", type: "POST",
                            data: { store_id: row.store_id, rnd: Math.random() },
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('module_sys/sys.store_add.php?r_store_id=' +row.store_id, "修改记录", 770, 490, f_save);
            } else {
		...
		function itemclick(item) {
			switch (item.id) {
				case "add":
                  ...
                  //alert("增加");
				   f_openWindow("module_sys/sys.store_add.php", "新增车队", 770, 300, f_save);
                  break;
				case "clearbtn":
					deleteRow();
                  //var selected = grid.getSelected();
    ...
    remark          varchar(300)        comment '仓库说明',
			*/	  
            ...
            manager=$("#maingrid4").ligerGrid({
                ...
                { display: '说明', name:'remark',minWidth:60 }
                ], pageSize:10,
            ...
            <table style='width: 960px' class="bodytable1">
                <tr>
					<td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>车队代码：</div>
						<div style='float: left;'>
						<input type='text' id='store_no' name='store_no' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
                    <td style='width:200px'>
