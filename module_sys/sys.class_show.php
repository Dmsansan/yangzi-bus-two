...
　　 }
	...
	function deleteRow()
       { 
			...
			if (row) {
                $.ligerDialog.confirm("记录删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/v1/?menuid=101114&cmd=del", type: "POST",
                            data: { class_id: row.class_id, rnd: Math.random() },
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('module_sys/sys.class_add.php?r_class_id=' +row.class_id, "修改记录", 770, 490, f_save);
            } else {
		...
		function itemclick(item) {
			switch (item.id) {
				case "add":
                  ...
                  //alert("增加");
				   f_openWindow("module_sys/sys.class_add.php", "新增轮胎层级", 770, 300, f_save);
                  break;
				case "clearbtn":
					deleteRow();
                  //var selected = grid.getSelected();
	  ...
	  function f_reload() {
		...
		function doserch() {
			...
			manager.set({url:'../ajaction/v1/?menuid=101114&cmd=qry&'+serchtxt});
			manager.loadData(true);
        }
    ...
    remark          varchar(200)        comment '备注',
			...
			*/	  
            ...
            manager=$("#maingrid4").ligerGrid({
                checkbox: false,
                columns: [
                { display: '层级编号', name:'class_id', align: 'left', width: 60 },
                { display: '层级代码', name:'class_no', width:150,isSort:false },
				{ display: '层级名称', name:'class_name', width: 150 },
				{ display: '备注',name:'remark',width: 300}
				], pageSize:10,
                url:'../ajaction/v1/?menuid=101114&cmd=qry&t=1',
            ...
            <table style='width: 960px' class="bodytable1">
                <tr>
                    <td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>层级代码：</div>
						<div style='float: left;'>
						<input type='text' id='class_no' name='class_no' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
					<td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>层级名称：</div>
						<div style='float: left;'>
						<input type='text' id='class_name' name='class_name' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
                    <td style='align:left'>
