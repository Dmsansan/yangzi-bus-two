...
　　 }
	...
	function deleteRow()
       { 
			...
			if (row) {
                $.ligerDialog.confirm("记录删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/v1/?menuid=101113&cmd=del", type: "POST",
                            data: { norms_id: row.norms_id, rnd: Math.random() },
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('module_sys/sys.norms_add.php?r_norms_id=' +row.norms_id, "修改记录", 770, 490, f_save);
            } else {
		...
		function itemclick(item) {
			switch (item.id) {
				case "add":
                  ...
                  //alert("增加");
				   f_openWindow("module_sys/sys.norms_add.php", "新增轮胎规格", 770, 300, f_save);
                  break;
				case "clearbtn":
					deleteRow();
                  //var selected = grid.getSelected();
	  ...
	  function f_reload() {
		...
		function doserch() {
			...
			manager.set({url:'../ajaction/v1/?menuid=101113&cmd=qry&'+serchtxt});
			manager.loadData(true);
        }
            ...
            <table style='width: 960px' class="bodytable1">
                <tr>
                    <td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>规格代码：</div>
						<div style='float: left;'>
						<input type='text' id='norms_no' name='norms_no' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
					<td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>规格名称：</div>
						<div style='float: left;'>
						<input type='text' id='norms_name' name='norms_name' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>
                    <td style='align:left'>
