...
　　 }
	...
	function deleteRow()
       { 
			...
			if (row) {
                $.ligerDialog.confirm("记录删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "../ajaction/v1/?menuid=101111&cmd=del", type: "POST",
                            data: { terminal_id: row.terminal_id, rnd: Math.random() },
	   ...
	   function edit() {
            ...
            var row = manager.getSelectedRow();
            if (row) {
                f_openWindow('module_sys/sys.terminal_add.php?r_terminal_id=' +row.terminal_id, "修改记录", 770, 490, f_save);
            } else {
		...
		function itemclick(item) {
			switch (item.id) {
				case "add":
                  ...
                  //alert("增加");
				   f_openWindow("module_sys/sys.terminal_add.php", "新增手持终端", 770, 300, f_save);
                  break;
				case "clearbtn":
					deleteRow();
                  //var selected = grid.getSelected();
	  ...
	  function f_reload() {
		...
		function doserch() {
			...
			manager.set({url:'../ajaction/v1/?menuid=101111&cmd=qry&'+serchtxt});
			manager.loadData(true);
        }
    ...
    remark          varchar(300)        comment '终端备注',
			*/	  
            ...
            manager=$("#maingrid4").ligerGrid({
                checkbox: false,
                columns: [
                { display: '终端编号', name:'terminal_id', align: 'left', width: 60 },
                { display: '终端IP', name:'terminal_ip', width:150,isSort:false },
				{ display: '归属车队', name:'store_name', width: 150,align:'left' },
				{ display: '最后一次登陆时间',name:'last_stamp',width: 120,align:'left'},
				{ display: '登陆用户',name:'admin_name',width: 100,align:'left'},
				{ display: '备注',name:'remark',width: 300,align:'left'}
				], pageSize:10,
                url:'../ajaction/v1/?menuid=101111&cmd=qry&t=1',
            ...
            <table style='width: 960px' class="bodytable1">
                <tr>
                    <td style='width:200px'>
                        <div style='float: left; text-align: right; width: 60px;'>终端IP：</div>
						<div style='float: left;'>
						<input type='text' id='terminal_ip' name='terminal_ip' ltype='text' ligerui='{width:120}' />
						</div>
                    </td>					
                    <td style='align:left'>
